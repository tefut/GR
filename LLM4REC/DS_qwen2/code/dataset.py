...
class CLDataset(Dataset):
    ...
    def __getitem__(self, idx):
        index_x, song_id_x, info_x, cluster_id_x, index_y, song_id_y, info_y, cluster_id_y = self.df.iloc[idx].tolist()
...
class CLCollator:
    ...
    def __call__(self, batch) -> dict:
        ...
        token_ids, attention = self.tokenizer.batch_encode_plus(describs,
                                                                truncation=True,
                                                                padding=True,
...
class PredictDataset(Dataset):
    ...
    def __getitem__(self, idx):
        index, song_id, info, cluster_id = self.df.iloc[idx].tolist()
        return int(index), song_id, info
...
class PredictCollator:
    ...
    def __call__(self, batch) -> dict:
        item_ids, song_ids, describs = zip(*batch)
        token_ids, attention = self.tokenizer.batch_encode_plus(describs,
                                                                truncation=True,
                                                                padding=True,
                                                                max_length=self.max_length,
