...
from modeling.generic.utils._model_register import register_model_cls
...
class FeatureBasedLossMask(BaseModel):
    ...
    def forward(self, model_inputs: Dict[str, torch.Tensor]):
        ...
        if self.mode == "exclude":
            ...
            raise ValueError("mode should be either include or exclude.")
...
class AGFeatureBasedLossMask(BaseModel):
    ...
    def forward(self, model_inputs: Dict[str, torch.Tensor]) -> Tuple[str, torch.Tensor]:
        feature_id = model_inputs[self.id_name]
        ...
        if self.mode == "exclude":
            ...
            condition = (
                    (feature_id == self.target_value)
                    & (feature_id_2 == self.target_value_2)
                    & torch.isin(feature_id_3, torch.tensor(self.target_value_3).to(feature_id.device))
            )  # (64,400)
            mask = torch.ones_like(feature_id, dtype=torch.bool).to(feature_id.device)
            # 如果有符合条件的索引，进行随机采样
            if condition.any():
            ...
            raise ValueError("loss_mask_conf.mode should be either include or exclude.")