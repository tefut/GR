...
from ..initialization import init_mlp_xavier_weights_zero_bias
...
class MoLGatingFn(torch.nn.Module):
    ...
    def __init__(
        ...
        if (self._context_only_partial_module is None and
                self._item_only_partial_module is None and
                self._ci_partial_module is None):
            raise ValueError(
...
class MoLSimilarity(torch.nn.Module):
    """
    Implements MoL (Mixture-of-Logits) learned similarity_module in
    Revisiting Neural Retrieval on Accelerators (https://arxiv.org/abs/2306.04039, KDD'23).
    """
    ...
    def __init__(
        ...
        self._lnx_num_batches = None
        self._B = None
        self._A = None
        self._lnx_estimator_num_elements = None
...
def create_mol_interaction_module(
    ...
    interaction_module_debug_str = (
            ...
            + (
                f"-q{query_hidden_dim}d{query_dropout_rate}geglu" if query_hidden_dim > 0 else
                f"-qd{query_dropout_rate}")
            + (
