...
def load_lora_config(train_config):
...
def predict(args):
    ...
    item_embeds = pickle.load(open(args.item_embedding_file, 'rb'))
    input_dim = train_config.get("input_dim", 64)
    output_dim = train_config.get("output_dim", 32)
    embedding_mode = train_config.get("embedding_mode", "pooled_output")
    data_collator = PredictCollator(args.base_model, args.cache_dir, max_length)
    dataloader = DataLoader(dataset, batch_size=batch_size, collate_fn=data_collator)

    # 构造模型
    lora_config = load_lora_config(train_config)
    ...
    model = SimCIE(
        ...
        save_path=args.finetune_model_path,
        embedding_mode=embedding_mode
    )

    # 加载模型参数
    device = torch.device("npu:0" if torch.npu.is_available() else "cpu")
    state_dict = torch.load(os.path.join(args.finetune_model_path, "adapter.pth"))
    new_state_dict = {}
    ...
    item_embeddings_output = write_to_file(args.output_file, 'w')
    ...
    with torch.no_grad():
        for data in tqdm(dataloader):
            ...
            index_ids = data["index_ids"].to(device)
            item_ids = data["item_ids"]
            with autocast():
                output_embedding = model(inputs, inputs_mask, index_ids)
            item_embeddings = output_embedding.cpu().numpy()

            for sid, item_embed in zip(item_ids, item_embeddings):
                item_embed = item_embed / np.linalg.norm(item_embed)
                emb = ",".join(map(str, item_embed.tolist()))
                item_embeddings_output.write(sid + "|" + emb + "\n")
